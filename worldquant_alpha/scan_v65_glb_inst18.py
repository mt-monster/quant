...
LOG_PATH = os.path.join(_HERE, "results", "v65_glb_inst18.log")
...
BATCH_SIZE = 8
N_LANES = int(os.environ.get("V65_LANES", "4"))  # USA PPA: lanes=4 sweet spot
COOLDOWN = float(os.environ.get("V53_COOLDOWN", str(DEFAULT_COOLDOWN_SEC)))
CKPT = os.path.join(_HERE, "results", "v65_glb_inst18_checkpoint.json")
READY = os.path.join(_HERE, "results", "manual_submit_ready.json")
DIVERSITY = os.path.join(_HERE, "results", "v65_diversity_report.jsonl")
DATASET = "institutions18"
...
STYLES: Dict[str, List[Tuple[str, str]]] = {
    # ---- v65a: institutions18 fund ownership (ji gou chi cang, MATRIX, yu jia ge zheng jiao) ----
    # cur vs pre holding -> ji gou lei ji/jian chi; fang xiang: lei ji(cur>pre)=+
    # A) holding rank change (zhu gong: accumulation)
    "chg": [
        ("chgS_hold", "group_rank(inst18_fundownershipv2_cur_holding, market) - group_rank(inst18_fundownershipv2_pre_holding, market)"),
        ("chgS_mv", "group_rank(inst18_fundownershipv2_cur_mv, market) - group_rank(inst18_fundownershipv2_vm_erp, market)"),
        ("chgN_hold", "group_rank(inst18_fundownershipv2_pre_holding, market) - group_rank(inst18_fundownershipv2_cur_holding, market)"),
    ],
    # B) raw diff rank (jian hao spread nei jian)
    "dif": [
        ("difP_hold", "group_rank(inst18_fundownershipv2_cur_holding - inst18_fundownershipv2_pre_holding, market)"),
        ("difP_mv", "group_rank(inst18_fundownershipv2_cur_mv - inst18_fundownershipv2_vm_erp, market)"),
    ],
    # C) ownership level (pct/num held)
    "lvl": [
        ("lvlP_pct", "group_rank(inst18_fundownershipv2_pct_held, market)"),
        ("lvlP_num", "group_rank(inst18_fundownershipv2_num_held, market)"),
        ("lvlN_pct", "group_rank(-inst18_fundownershipv2_pct_held, market)"),
        ("lvlN_num", "group_rank(-inst18_fundownershipv2_num_held, market)"),
    ],
    # D) smoothed holding change / pct delta (wen ding + ti TVR)
    "sm": [
        ("smS_hold", "group_rank(ts_mean(inst18_fundownershipv2_cur_holding, 22), market) - group_rank(ts_mean(inst18_fundownershipv2_pre_holding, 22), market)"),
        ("smP_pctd", "group_rank(ts_delta(inst18_fundownershipv2_pct_held, 22), market)"),
    ],
    # E) country skeleton tan zhen
    "cty": [
        ("ctyS_hold", "group_rank(inst18_fundownershipv2_cur_holding, country) - group_rank(inst18_fundownershipv2_pre_holding, country)"),
        ("ctyP_pct", "group_rank(inst18_fundownershipv2_pct_held, country)"),
    ],
}

UNIVERSES = ["TOP3000", "TOP2000", "TOP1000"]  # USA: standard TOP universes
DECAYS = [3, 6]
NEUTS = ["SUBINDUSTRY", "INDUSTRY", "SECTOR"]  # USA single-region: industry-level neut
TRUNCS = [0.02]
...
def build_variants() -> List[Dict[str, Any]]:
    ...
    # 探索优先级: 先 TOP3000+SUBINDUSTRY (USA 单市场去行业噪声)
    def prio(v):
        s = v["settings"]
        sc = 0
        if s["universe"] == "TOP3000": sc += 4
        if s["universe"] == "TOP2000": sc += 2
        if s["neutralization"] == "SUBINDUSTRY": sc += 3
        if s["neutralization"] == "INDUSTRY": sc += 1
        return -sc
    out.sort(key=prio)
    ...
    while True:
        ...
        for item in raw:
            ...
            info = {
                ...
                "region": "USA", "submitted": False,
                "tags": ["v65", DATASET, "USA_D1", "READY_MANUAL", "NO_SUBMIT"],
            }
...
def main():
    ...
    logger.info("V65 GLB institutions18 fund-ownership | %d variants | lanes=%d | %s",
