            ...
            try:
                ...
                if result:
                    ...
                    if db_id and isinstance(db_id, int):
                        ...
                        logger.info(f"[{thread_name}] [SAVE] 保存Alpha结果，ID={db_id}, Sharpe={sharpe:.4f}, Fitness={fitness:.4f}, Color={color}")
                        save_alpha_result(
                            alpha_id=db_id,
                        ...
                        platform_id = result.get('id')
                        self_corr = result.get('self_corr')
                        logger.debug(f"[{thread_name}] 回测完成，平台ID: {platform_id}, Sharpe: {sharpe:.4f}, Self_Corr: {self_corr}, 未保存到数据库（无DB ID）")
