> **数据快照**: 2026-07-26 18:24 GMT+8 ｜ **数据源**: `results/*_checkpoint.json`(权威) + `*_progress_*.log`(实时) + `tri_track_undug_results.csv`
...
| 指标 | 数值 | 说明 |
|---|---|---|
| 累计回测次数 | **13,747** | 全部 32 个 checkpoint 合计 |
| IS 廉价闸门通过 | **43** (31 PASS_CHEAP + 4 CHECK_PENDING) | 仅研究仿真 IS 闸通过，非「可提交」 |
...
每一级都是一道硬闸门：**IS 廉价闸门**筛掉 13,704 次（13,704/13747=99.7% 被筛）；**生产相关性关**仅 1 个通过；**平台提交**为 0。43 个候选 ≠ 可提交。
...
| 已提交 alpha | 累计 7664 次 (含研究仿真) | 64 alpha 已提交并完成 |
...
| earningscall_embed | 312/320 (97.5%) | 🟡 **1.05** | ~0 α/hr | 0 min | **?** | 🔴 0 候选 |
| earningscall_sentiment | 320/320 (100.0%) | 🟢 **1.67** | ~0 α/hr | 0 min | **?** | 🔴 0 候选 |
| equity_forum_data | 304/320 (95.0%) | 🟡 **1.26** | ~0 α/hr | 0 min | **?** | 🔴 0 候选 |
...
| news_sentiment_dl | 296/320 (92.5%) | 🔴 **0.68** | ~0 α/hr | 0 min | **?** | 🔴 0 候选 |
...
| 🛡️ tri_track_undug | 64 alpha | **07-27 23:42** | 进度日志推算 |
...
| 21 | 🚢 ds_equity_forum_data_tri_equity_forum_da | 304 | 0 | 🟡 **1.26** | - | gate_S/F/M/Ret |
...
| 26 | 🚢 ds_earningscall_embed_tri_earningscall_e | 312 | 0 | 🟡 **1.05** | - | gate_S/F/M/Ret |
...
| 41 | 🚢 ds_news_sentiment_dl_tri_news_sentiment_ | 296 | 0 | 🔴 **0.68** | - | gate_S/F/M/Ret |
...
| 已提交 alpha | **64** 个  |
| 提交结果 | ✅ submitted=64 / ❌ failed=0 |
| 最佳 Sharpe | **1.58** |
| 分轨分布 | explore 35 / improve 29 / misc 0 |
| 时间范围 | ? ~ ? |
...
| 预期完成 | **07-27 23:42** (基于每分片 ~300s × 6 剩余 / CONCURRENCY=3, 粗估) |
...
| Ret(收益) | **10,337** | 20.7% |  |
| TVR(换手率) | **5,110** | 10.2% |  |
| submit_failed | **1,792** | 3.6% |  |
| PF:子宇宙Sharpe | **304** | 0.6% | 主因 PF:LOW_SUB_UNIVERSE_SHARPE (219次) |

> **PF:子宇宙 Sharpe 是头号平台失败闸门** (304 次)，印证'在子宇宙层面优化中性化/约束'是攻坚方向。IS 指标失败(夏普/拟合/换手/收益)合计 47,938 次，submit_failed 1,792 次(多为研究仿真本地拒，非平台 429)。
...
| 🔶 仍需进一步验证 | **42** | 35 个候选缺生产仿真(OOS)+submittable+submit |

> ⚠️ **实话实说**：全部 43 个候选，**1 个已提交、35 个不可提交**。`YPgAa3WR` 已验证 IS✅ + 生产相关性(0.5325)✅ + 风险中性✅ + 稳健性✅，已成功提交至 WQ 平台(status=ACTIVE)。其余 35 个均缺平台生产仿真(OOS)硬闸门——`/check` 返回空，提交即被静默丢弃。`PASS_CHEAP` ≈ 1/4 路程，`CHECK_PENDING` ≈ 1/2 路程。
...
---
*报告由 `build_md_report.py` 从真实 checkpoint/progress/CSV 文件程序化生成 · 快照 2026-07-26 18:24 GMT+8 · 数字均来自文件实测，未编造。*
