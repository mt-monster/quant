...
def backtest_one(api, expr, dataset_id, sn, min_sharpe, min_fitness, prod_corr_wait):
    logger.info("SIM #%d [%s] %s...", sn, dataset_id, expr[:80])
    try:
        res = api.run_backtest(expr, settings=BACKTEST_SETTINGS.copy())
...
def main() -> None:
    ...
    logger.info("=" * 72)
    logger.info("EUR D1 RAM PPA mining | dataset=%s | target=%d", args.dataset, TARGET_COUNT)
    logger.info("=" * 72)
