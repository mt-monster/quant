#!/usr/bin/env python3
"""V33: KOR PPA 扫描 — chart_cnn_alpha (DL预测) + pattern_scores (新信号对).

V32 失败原因: pattern_scores 同类模式内部 spread (mean vs min) 是离散度信号,
非方向性信号; 最佳 Sharpe=0.680 远低于 1.58 阈值, 21/21 Ladder FAIL.

V33 改进:
  Part A: chart_cnn_alpha — CNN 预测排名直通信号 (DL模型输出, 最强)
  Part B: chart_cnn_alpha — CNN 概率桶 spread (高收益概率 - 低收益概率)
  Part C: pattern_scores — 跨模式方向性 spread (牛 vs 熊, 新信号对)
  Part D: pattern_scores — 单字段直通信号 (短 zscore 窗口)
...
PROGRESS_LOG_PATH = os.getenv("PROGRESS_LOG_PATH", os.path.join(_here, "results", f"v33_kor_progress_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"))
...
# =====================================================================
# Part A: chart_cnn_alpha — CNN Predicted Rank (DL模型预测排名, 直通信号)
# =====================================================================
# CNN 模型直接预测股票排名, quintile q1(最低)-q5(最高)
# 直通使用: rank(ts_backfill(field, 66)) — 已是排名, 不需额外 zscore
CNN_RANK_FIELDS = [
    "cnn_predicted_rank_q5_60d_equity2b2",    # 最高分位, 60天回看
    "cnn_predicted_rank_q5_100d_malta3",       # 最高分位, 100天回看
    "cnn_predicted_rank_q4_60d_equity2b2",     # 第四分位, 60天
    "cnn_predicted_rank_q3_60d_equity2b2",     # 中位分位, 60天
    "cnn_predicted_rank_q1_60d_equity2b2",     # 最低分位 (做空信号)
    "cnn_predicted_rank_q5_50d_equity2b2",     # 最高分位, 50天
]

# =====================================================================
# Part B: chart_cnn_alpha — CNN Prob Bucket Spread (概率方向信号)
# =====================================================================
# bucket0=最低收益, bucket4=最高收益
# spread = prob(高收益) - prob(低收益) = 方向性信号
CNN_PROB_PAIRS = [
    ("cnn_predicted_prob_q5_bucket4_60d_equity2b2", "cnn_predicted_prob_q5_bucket0_60d_equity2b2", "q5_b4vs0_60d"),
    ("cnn_predicted_prob_q5_bucket4_50d_equity2b2", "cnn_predicted_prob_q5_bucket0_50d_equity2b2", "q5_b4vs0_50d"),
    ("cnn_predicted_prob_q5_bucket3_60d_equity2b2", "cnn_predicted_prob_q5_bucket1_60d_equity2b2", "q5_b3vs1_60d"),
    ("cnn_predicted_prob_q5_bucket2_60d_equity2b2", "cnn_predicted_prob_q5_bucket0_60d_equity2b2", "q5_b2vs0_60d"),
]

# =====================================================================
# Part C: pattern_scores — 跨模式方向性 spread (新信号对)
# =====================================================================
# V32 用同类模式内部 spread (mean vs min) = 离散度信号, 预测力弱
# V33 改用 跨模式方向性 spread (牛 vs 熊) = 方向性信号
PS_DIR_PAIRS = [
    # 上升三角形 (牛) vs 下降三角形 (熊)
    ("asc_triangle_mean_simscore_lookback120", "desc_triangle_up_mean_simscore_lookback120", "asc_vs_desc_tri"),
    # 上升对称三角形 (牛) vs 下降对称三角形 (熊)
    ("avg_similarity_upward_symmetrical_triangle", "avg_similarity_downward_symmetrical_triangle", "up_vs_down_sym"),
    # V反转底部 (牛) vs V反转顶部 (熊)
    ("avg_similarity_v_reversal_bottom_120", "reversal_v_top_mean_simscore_lookback120", "vbot_vs_vtop_rev"),
    # V持续底部 (牛) vs V持续顶部 (熊)
    ("cont_v_bottom_mean_simscore_lookback120", "cont_v_top_max_simscore_lookback120", "vbot_vs_vtop_cont"),
]

# =====================================================================
# Part D: pattern_scores — 单字段直通信号 (短 zscore 窗口)
# =====================================================================
# 直接用单个模式分数 (不做 spread), zscore 窗口 126 (比 V32 的 189 更短)
PS_SINGLE_FIELDS = [
    "asc_triangle_mean_simscore_lookback120",       # 上升三角形 (牛市模式)
    "avg_similarity_v_reversal_bottom_120",          # V反转底部 (牛市反转)
    "avg_similarity_upward_breakaway_gap_120",       # 向上跳空 (牛市信号)
    "cont_v_bottom_mean_simscore_lookback120",       # V持续底部 (牛市持续)
]
...
# --- Part A: CNN Rank 直通信号 ---
for field in CNN_RANK_FIELDS:
    A = f"ts_backfill({field}, 66)"
    fund = f"rank({A})"
    for w in [0.0, 0.15, 0.25, 0.35]:
        ret_expr = f"scale(-rank(ts_zscore(returns, 42)))"
        for decay in [4, 5, 6]:
            sig_name = field.replace("cnn_predicted_rank_", "rank_")
            label = f"KOR_cnn_{sig_name}_w{w}_d{decay}"
            if w == 0:
                expr = f"scale({fund})"
            else:
                expr = f"scale({fund}) + {ret_expr} * {w}"
            VARIANTS.append((label, expr,
                {"decay": decay, "region": "KOR", "universe": "TOP600"}, "KOR"))

# --- Part B: CNN Prob Bucket Spread ---
for fA, fB, sig_name in CNN_PROB_PAIRS:
    A = f"ts_backfill({fA}, 66)"
    B = f"ts_backfill({fB}, 66)"
    SPREAD = f"subtract(ts_mean({A}, 22), ts_mean({B}, 22), filter=true)"
    fund_plain = f"rank(ts_zscore({SPREAD}, 189))"
    fund_gz = f"rank(group_zscore(ts_zscore({SPREAD}, 189), industry))"
    for w in [0.15, 0.25, 0.35]:
        ret_expr = f"scale(-rank(ts_zscore(returns, 42)))"
        for decay in [4, 5, 6]:
            label = f"KOR_cnnprob_{sig_name}_plain_w{w}_d{decay}"
            VARIANTS.append((label, f"scale({fund_plain}) + {ret_expr} * {w}",
                {"decay": decay, "region": "KOR", "universe": "TOP600"}, "KOR"))
            label = f"KOR_cnnprob_{sig_name}_gzi_w{w}_d{decay}"
            VARIANTS.append((label, f"scale({fund_gz}) + {ret_expr} * {w}",
                {"decay": decay, "region": "KOR", "universe": "TOP600"}, "KOR"))

# --- Part C: Pattern Scores 跨模式方向性 spread ---
for fA, fB, sig_name in PS_DIR_PAIRS:
    A = f"ts_backfill({fA}, 66)"
    B = f"ts_backfill({fB}, 66)"
    SPREAD = f"subtract(ts_mean({A}, 22), ts_mean({B}, 22), filter=true)"
    fund_plain = f"rank(ts_zscore({SPREAD}, 189))"
    ...
    for w in [0.15, 0.25, 0.35]:
        ret_expr = f"scale(-rank(ts_zscore(returns, 42)))"
        for decay in [4, 5, 6]:
            label = f"KOR_ps_{sig_name}_plain_w{w}_d{decay}"
            VARIANTS.append((label, f"scale({fund_plain}) + {ret_expr} * {w}",
                {"decay": decay, "region": "KOR", "universe": "TOP600"}, "KOR"))
            label = f"KOR_ps_{sig_name}_gzi_w{w}_d{decay}"
            VARIANTS.append((label, f"scale({fund_gz}) + {ret_expr} * {w}",
                {"decay": decay, "region": "KOR", "universe": "TOP600"}, "KOR"))

# --- Part D: Pattern Scores 单字段直通 (短 zscore 窗口) ---
for field in PS_SINGLE_FIELDS:
    A = f"ts_backfill({field}, 66)"
    fund_plain = f"rank(ts_zscore({A}, 126))"
    fund_gz = f"rank(group_zscore(ts_zscore({A}, 126), industry))"
    for w in [0.0, 0.15, 0.25]:
        ret_expr = f"scale(-rank(ts_zscore(returns, 42)))"
        for decay in [4, 5]:
            sig_name = field[:20]
            label = f"KOR_pssingle_{sig_name}_plain_w{w}_d{decay}"
            if w == 0:
                expr = f"scale({fund_plain})"
            else:
                expr = f"scale({fund_plain}) + {ret_expr} * {w}"
            VARIANTS.append((label, expr,
                {"decay": decay, "region": "KOR", "universe": "TOP600"}, "KOR"))
            label = f"KOR_pssingle_{sig_name}_gzi_w{w}_d{decay}"
            if w == 0:
                expr = f"scale({fund_gz})"
            else:
                expr = f"scale({fund_gz}) + {ret_expr} * {w}"
            VARIANTS.append((label, expr,
                {"decay": decay, "region": "KOR", "universe": "TOP600"}, "KOR"))

logger.info("V33 KOR scan | %d variants | log: %s", len(VARIANTS), PROGRESS_LOG_PATH)
...
api = WqApiSimple()
results = []; candidates = []
pl = ProgressLogger(total_steps=len(VARIANTS), log_path=PROGRESS_LOG_PATH, task_name="v33_kor_region", emit_interval_sec=15.0, max_recent=5)
pl.start(meta={"regions":["KOR"],"datasets":["chart_cnn_alpha","pattern_scores"],"variants":len(VARIANTS)})
try:
...
logger.info("V33 complete | %d done, %d valid candidates (NOT submitted)", len(results), len(candidates))
for r in results[:20]:
    tvr = r.get("tvr"); tvr_s = f"{tvr*100:.1f}%" if tvr else "?"
    pc = r.get("prod_corr","?")
    reg = r.get("region","?")
    logger.info("  %-50s %s S=%-6.3f F=%-6.3f TVR=%-7s LAD=%-5s(%s) PC=%s %s",
                f"{reg}/{r['label']}", "CANDIDATE" if r.get("passed") else "FAIL",
                r.get("sharpe",0), r.get("fitness",0), tvr_s,
                r.get("ladder","?"), r.get("ladder_result","?"), pc,
                "; ".join(r.get("fails",[]))[:40])

out_path = os.path.join(_here, "results", f"scan_v33_kor_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
