#!/usr/bin/env python3
"""V42: USA D1 social_sent_score (未点亮) — multi-sim 8.

风格: 社交舆情意见分/变化 (异于 insider 动量 / 地产 / 盈利波动).
...
BATCH_SIZE = 8
BATCH_COOLDOWN_SEC = float(os.environ.get("V42_COOLDOWN", "45"))
PROGRESS_LOG_PATH = os.path.join(_HERE, "results", f"v42_social_progress_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
CKPT_PATH = os.path.join(_HERE, "results", "v42_social_checkpoint.json")
FOUND_PATH = os.path.join(_HERE, "results", f"scan_v42_social_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
READY_PATH = os.path.join(_HERE, "results", "manual_submit_ready.json")
DATASET = "social_sent_score"
...
FIELDS = ["standardized_opinion_score", "opinion_score_numeric", "opinion_score_delta"]
PAIRS = [
    ("standardized_opinion_score", "opinion_score_delta", "std_vs_delta"),
    ("opinion_score_numeric", "opinion_score_delta", "num_vs_delta"),
]
...
def build_variants():
    ...
    for f in FIELDS:
        short = f.replace("opinion_score_", "op_").replace("standardized_", "std_")[:14]
        for uni in UNIVERSES:
            for decay in DECAYS:
                for neut in NEUTS:
                    for trunc in TRUNCS:
                        add(f"z_{short}_{uni}_d{decay}_{neut[:3]}_t{int(trunc*100)}", f"rank(ts_zscore(ts_mean(vec_avg({f}), 22), 126))", uni, decay, neut, "zscore", f, trunc)
                        add(f"nz_{short}_{uni}_d{decay}_{neut[:3]}_t{int(trunc*100)}", f"-rank(ts_zscore(ts_mean(vec_avg({f}), 22), 126))", uni, decay, neut, "zscore_flip", f, trunc)
                        add(f"gz_{short}_{uni}_d{decay}_{neut[:3]}_t{int(trunc*100)}", f"rank(group_zscore(ts_zscore(ts_mean(vec_avg({f}), 22), 189), industry))", uni, decay, neut, "group_z", f, trunc)
                    ...
                    add(f"tr_{short}_{uni}_d{decay}_{neut[:3]}", f"rank(ts_rank(vec_avg({f}), 66))", uni, decay, neut, "ts_rank", f)
        ...
        for uni in UNIVERSES:
            for decay in DECAYS:
                for neut in NEUTS:
                    ...
                    add(f"spgz_{tag}_{uni}_d{decay}_{neut[:3]}", f"group_zscore(ts_zscore(subtract(vec_avg({a}), vec_avg({b})), 126), industry)", uni, decay, neut, "spread_gz", f"{a}|{b}")
...
def main():
    ...
    def prio(v):
        s = 0
        if "standardized" in str(v["field"]): s += 8
        if v["style"] in ("group_z", "zscore"): s += 5
        if v["settings"]["universe"] == "TOP3000": s += 3
        if v["settings"]["neutralization"] == "SECTOR": s += 2
        return -s
    ...
    logger.info("V42 %s | %d | NO SUBMIT", DATASET, len(VARIANTS))
    ...
    pl = ProgressLogger(total_steps=len(VARIANTS), log_path=PROGRESS_LOG_PATH, task_name="v42_social", emit_interval_sec=15.0, max_recent=8)
    ...
    for r in survivors:
        ...
        if pc_val is None or pc_val >= MAX_PROD_CORR:
            ...
            continue
        info = {"dataset": DATASET, "style": "social_opinion", "pid": pid, "label": label, "expr": expr, "sharpe": r["sharpe"], "fitness": r["fitness"], "tvr": r["tvr"], "margin": r["margin"], "prod_corr": pc_val, "risk_neut": rn_stats, "robust": robust, "settings": settings, "submitted": False}
        set_alpha_props(api, pid, f"v42_{label}", ["v42", DATASET, "USA_D1", "READY_MANUAL", "NO_SUBMIT"])
        found_alphas.append(info)
        append_ready({**info, "margin_bp": (r["margin"] or 0) * 10000, "tags": ["v42", DATASET, "USA_D1", "READY_MANUAL"]})
