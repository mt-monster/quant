#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""V54: GLB/D1 未点亮金字塔 sentiment21 (AI Sentiment Score Data, pm=1.6) 挖掘.

前序: intraday_pv_feats 已深挖 713 变体/21 风格全证伪 (margin-2Y-EMEA 三角无缝隙), 切换至
情绪类数据集 (收益来源与微观结构正交, pm=1.6 全场最高).
字段结构: snt21_{窗口组 base/2/3/4/5}{pos|neg|neut|dts}_{统计量}; 社区主矿脉=负面情绪族
(snt21_2neg_conf_up 449 alphas / snt21_2neg_std 271 / snt21_2neg_max 205, cov=0.83).
...
LOG_PATH = os.path.join(_HERE, "results", "v54_glb_sentiment21.log")
...
DIVERSITY = os.path.join(_HERE, "results", "v54_diversity_report.jsonl")
DATASET = "sentiment21"
...
# 方向未知 → P/N 双符号探测; 窗口 5/10/22 覆盖快慢
STYLES: Dict[str, List[Tuple[str, str]]] = {
    # A) 负面情绪水平 (社区主矿脉 449 alphas): 坏消息多→看空(N) or 过度恐慌反转(P)
    "neg_level": [
        ("nglN_w5", "-group_zscore(ts_mean(ts_backfill(snt21_2neg_conf_up, 66), 5), industry)"),
        ("nglN_w10", "-group_zscore(ts_mean(ts_backfill(snt21_2neg_conf_up, 66), 10), industry)"),
        ("nglN_w22", "-group_zscore(ts_mean(ts_backfill(snt21_2neg_conf_up, 66), 22), industry)"),
        ("nglP_w5", "group_zscore(ts_mean(ts_backfill(snt21_2neg_conf_up, 66), 5), industry)"),
        ("nglP_w10", "group_zscore(ts_mean(ts_backfill(snt21_2neg_conf_up, 66), 10), industry)"),
        ("nglP_w22", "group_zscore(ts_mean(ts_backfill(snt21_2neg_conf_up, 66), 22), industry)"),
    ],
    # B) 负面情绪分歧度 (std, 271 alphas): 情绪不确定性溢价
    "neg_disp": [
        ("ngdN_w10", "-group_zscore(ts_mean(ts_backfill(snt21_2neg_std, 66), 10), industry)"),
        ("ngdN_w22", "-group_zscore(ts_mean(ts_backfill(snt21_2neg_std, 66), 22), industry)"),
        ("ngdP_w10", "group_zscore(ts_mean(ts_backfill(snt21_2neg_std, 66), 10), industry)"),
        ("ngdP_w22", "group_zscore(ts_mean(ts_backfill(snt21_2neg_std, 66), 22), industry)"),
    ],
    # C) 净情绪 spread (pos_mean - neg_mean, 2字段同量纲减法)
    "net_sent": [
        ("nssP_w5", "group_zscore(ts_mean(ts_backfill(snt21_2pos_mean - snt21_2neg_mean, 66), 5), industry)"),
        ("nssP_w22", "group_zscore(ts_mean(ts_backfill(snt21_2pos_mean - snt21_2neg_mean, 66), 22), industry)"),
        ("nssN_w5", "-group_zscore(ts_mean(ts_backfill(snt21_2pos_mean - snt21_2neg_mean, 66), 5), industry)"),
        ("nssN_w22", "-group_zscore(ts_mean(ts_backfill(snt21_2pos_mean - snt21_2neg_mean, 66), 22), industry)"),
    ],
    # D) 负面冲击边际变化 (neg_max delta): 新闻冲击事件风格, 高换手
    "neg_shock": [
        ("ngkN_w5", "-group_zscore(ts_delta(ts_backfill(snt21_2neg_max, 66), 5), industry)"),
        ("ngkN_w10", "-group_zscore(ts_delta(ts_backfill(snt21_2neg_max, 66), 10), industry)"),
        ("ngkP_w5", "group_zscore(ts_delta(ts_backfill(snt21_2neg_max, 66), 5), industry)"),
        ("ngkP_w10", "group_zscore(ts_delta(ts_backfill(snt21_2neg_max, 66), 10), industry)"),
    ],
    # E) v54b: nssN 定向强化. 证据: nssN_w5_COUN_d3 S=0.70 最佳, w22 M=3.0bp (margin 随窗口增),
    #    COUN 全面优于 SUBI. 杠杆: 长窗口(w44/w66)提margin / ts_zscore时序标准化 / winsorize / rank.
    "nss_boost": [
        ("nsbN_w44", "-group_zscore(ts_mean(ts_backfill(snt21_2pos_mean - snt21_2neg_mean, 66), 44), industry)"),
        ("nsbN_w66", "-group_zscore(ts_mean(ts_backfill(snt21_2pos_mean - snt21_2neg_mean, 66), 66), industry)"),
        ("nsbZ_w22", "-group_zscore(ts_zscore(ts_backfill(snt21_2pos_mean - snt21_2neg_mean, 66), 22), industry)"),
        ("nsbZ_w66", "-group_zscore(ts_zscore(ts_backfill(snt21_2pos_mean - snt21_2neg_mean, 66), 66), industry)"),
        ("nsbW_w22", "-group_zscore(winsorize(ts_mean(ts_backfill(snt21_2pos_mean - snt21_2neg_mean, 66), 22)), industry)"),
        ("nsbR_w22", "-group_rank(ts_rank(ts_backfill(snt21_2pos_mean - snt21_2neg_mean, 66), 22), industry)"),
    ],
    # F) v54b: 负面分歧度用 cov=1.00 全覆盖字段 snt21_2dts_gen_391 (即 neg std 反写版, 免 backfill),
    #    证据: ngdN_COUN S≈0.5, cov=0.83 字段缺口可能压低 S. 同时探 pos/neg 比率骨架.
    "ngd_clean": [
        ("gddN_w10", "-group_zscore(ts_mean(snt21_2dts_gen_391, 10), industry)"),
        ("gddN_w22", "-group_zscore(ts_mean(snt21_2dts_gen_391, 22), industry)"),
        ("gddN_w44", "-group_zscore(ts_mean(snt21_2dts_gen_391, 44), industry)"),
        ("gddZ_w22", "-group_zscore(ts_zscore(snt21_2dts_gen_391, 22), industry)"),
        ("nsrN_w22", "-group_zscore(ts_mean(ts_backfill(snt21_2pos_mean / snt21_2neg_mean, 66), 22), industry)"),
    ],
    # G) v54c: gddN 锐化. 证据: gddN_w22_MINV S=1.05-1.06 F=0.45-0.47 M=3.7-4.3bp (cov=1.0 干净字段
    #    比 cov=0.83 版 S 翻倍); w44 M=5.5-6.1bp (margin 随窗口增). TOPD 证伪只跑 MINV.
    #    杠杆: 中长窗口提 margin / decay_linear / winsorize / subindustry / neg-pos std 价差.
    "gdd_sharpen": [
        ("gd2N_w32", "-group_zscore(ts_mean(snt21_2dts_gen_391, 32), industry)"),
        ("gd2N_w66", "-group_zscore(ts_mean(snt21_2dts_gen_391, 66), industry)"),
        ("gd2Nd_w22", "-group_zscore(ts_decay_linear(snt21_2dts_gen_391, 22), industry)"),
        ("gd2Nd_w44", "-group_zscore(ts_decay_linear(snt21_2dts_gen_391, 44), industry)"),
        ("gd2Nw_w22", "-group_zscore(winsorize(ts_mean(snt21_2dts_gen_391, 22)), industry)"),
        ("gd2Nu_w22", "-group_zscore(ts_mean(snt21_2dts_gen_391, 22), subindustry)"),
        ("gdsN_w22", "-group_zscore(ts_mean(snt21_2dts_gen_391 - snt21_2dts_sop_380, 22), industry)"),
        ("gdsN_w44", "-group_zscore(ts_mean(snt21_2dts_gen_391 - snt21_2dts_sop_380, 44), industry)"),
    ],
    # H) v54c: nsr/nss 干净字段版. 证据: nsrN(脏字段) S=0.95-0.96 M=5.0-5.4bp —— 若干净字段
    #    复制 gdd 的翻倍效应则可过线. 免 backfill.
    "nsr_clean": [
        ("nr2N_w22", "-group_zscore(ts_mean(snt21_2pos_mean_384 / snt21_2neg_mean_390, 22), industry)"),
        ("nr2N_w44", "-group_zscore(ts_mean(snt21_2pos_mean_384 / snt21_2neg_mean_390, 44), industry)"),
        ("nr2N_w66", "-group_zscore(ts_mean(snt21_2pos_mean_384 / snt21_2neg_mean_390, 66), industry)"),
        ("ns2N_w22", "-group_zscore(ts_mean(snt21_2pos_mean_384 - snt21_2neg_mean_390, 22), industry)"),
    ],
}
...
# v54: 4 风格全部沿用全局默认 (MINVOL1M × d3/d6 × COUNTRY/SUBINDUSTRY)
# v54b: SUBI 已证伪只留 COUNTRY; TOPDIV3000 试高分红池提 margin; d6/d10 压 TVR
...
def main():
    ...
    logger.info("V54 GLB sentiment21 | %d variants | lanes=%d | %s",
