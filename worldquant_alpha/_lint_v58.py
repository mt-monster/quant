#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""v58 待跑表达式本地约束校验 (纯本地, 不调 API, 不 import scan 模块避免副作用).

检查项:
  1. 操作符数量 < 6
  2. 禁用算子: trade_when / add() / multiply()
  3. 单表达式数据集字段数 1-2
  4. 字段真实存在于 sentiment26 且 coverage > 0.90
...
SCAN = os.path.join(_HERE, "scan_v58_glb_snt26.py")
FIELDS_JSON = os.path.join(_HERE, "results", "glb_fields_sentiment26.json")
