> **数据快照**: 2026-07-26 18:36 GMT+8 ｜ **数据源**: `results/*_checkpoint.json`(权威) + `*_progress_*.log`(实时) + `tri_track_undug_results.csv`

> ⚠️ **提交验证最重要结论**：全部 **43** 个候选 Alpha 中，**1 个已正式提交**（`YPgAa3WR`，status=ACTIVE，dateSubmitted=2026-07-24，prod_corr=0.5325），剩余 **42** 个缺平台生产仿真(OOS)硬闸门验证、不可提交。`PASS_CHEAP` 仅表示本地廉价 IS 闸通过，不等于可提交。
...
| 指标 | 数值 | 说明 |
|---|---|---|
| 累计回测次数 | **13,771** | 全部 32 个 checkpoint 合计 |
| IS 廉价闸门通过 | **43** (39 PASS_CHEAP + 4 CHECK_PENDING) | 仅研究仿真 IS 闸通过，非「可提交」 |
...
每一级都是一道硬闸门：**IS 廉价闸门**筛掉 13,728 次（13,728/13771=99.7% 被筛）；**生产相关性关**仅 1 个通过；**平台提交**为 0。43 个候选 ≠ 可提交。
...
| 🛡️ tri_track_undug | 84 alpha | **07-27 23:29** | 进度日志推算 |
...
| 7 |  rescue_r3_web_lift | 40 | 1 | 🟢 **2.11** | 候选 | gate_S/F/M/Ret |
...
| 预期完成 | **07-27 23:29** (基于每分片 ~300s × 6 剩余 / CONCURRENCY=3, 粗估) |
...
> **PF:子宇宙 Sharpe 是头号平台失败闸门** (304 次)，印证'在子宇宙层面优化中性化/约束'是攻坚方向。IS 指标失败(夏普/拟合/换手/收益)合计 47,938 次，submit_failed 1,816 次(多为研究仿真本地拒，非平台 429)。

---
## 八、候选 Alpha 明细 (43 个，按 Sharpe 降序)
...
| 🔶 仍需进一步验证 | **42** | 42 个候选缺生产仿真(OOS)+submittable+submit |

> ⚠️ **实话实说**：全部 43 个候选，**1 个已提交、42 个不可提交**。`YPgAa3WR` 已验证 IS✅ + 生产相关性(0.5325)✅ + 风险中性✅ + 稳健性✅，已成功提交至 WQ 平台(status=ACTIVE)。其余 42 个均缺平台生产仿真(OOS)硬闸门——`/check` 返回空，提交即被静默丢弃。`PASS_CHEAP` ≈ 1/4 路程，`CHECK_PENDING` ≈ 1/2 路程。
...
1. **候选提交率 1/43**。仅 `YPgAa3WR` 已提交(ACTIVE)，其余 42 个缺平台 OOS 硬闸门。见第九章逐项审计。
...
5. **提交核查路线（按优先级）**：YPgAa3WR(最接近) → 4 个 CHECK_PENDING 等平台结果 → 39 个 PASS_CHEAP 排队验证。对 YPgAa3WR：跑 OOS → /check → submittable → submit(关 no_submit)；走通全流程后批量复制到其余候选。
...
---
*报告由 `build_md_report.py` 从真实 checkpoint/progress/CSV 文件程序化生成 · 快照 2026-07-26 18:36 GMT+8 · 数字均来自文件实测，未编造。*
