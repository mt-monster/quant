...
- **数据快照时间**: 2026-07-25 21:07:14 GMT+8
- **覆盖任务**: V33 -> V54 (历史链 V33-V45 + 主账号 trio V46 + 并发批次 V47-V54，共 25 个扫描任务)
- **累计回测次数(已完成链)**: 5667  |  **累计通过候选**: 1  |  **全链路最佳 Sharpe**: 2.65
- **机器级 Python 进程 (接触 WQ BRAIN): 16 个** = 2 扫描脚本 + 14 MCP 服务(交互式服务端回测宿主) + 0 watchdog + 0 tracker；其中 1 个扫描进程在飞、在飞进程已产出 ≈ 40 条回测结果、全局 429 = 0。MCP 服务发起的服务端任务(如 `set_RR11jN_`) 不写本地日志, 见 §9。
...
2. **零 429 实证**：当前 1 个进程在飞；历史累计提交 ≈5667 次回测，全链路 `submit_failed=0 / 429=0 / poll_timeout=0`。并发批次启动时间 **错峰分布在 08:39-08:51 (约12分钟)**，且各进程自带 submit_gate(>=18s/>=45s)，故瞬时提交浓度被压在 C=7 内——**证实"错峰 + 每进程闸门"可安全突破旧保守上限(<=6)**。
...
- 机器级 python 进程总数: **28** | 接触 WQ BRAIN: **16** = scan 2 + MCP服务 14 + watchdog 0 + tracker 0 | 其余 12 为编辑器/语言服务(idle)
- 命令行命中 `RR11jN`: **0** 个
...
| 36640 | scan_script | 07-25 21:06 | 5 | 追踪 - | - | - | - |  | 扫描脚本(本地进度日志见 §2/§3) |

> **第一视角核验结论**：机器级枚举到 16 个接触 WQ BRAIN 的 python 进程——2 个 scan 脚本(V46-V53 在飞, 带本地进度日志) + 14 个 MCP 服务(交互式服务端回测宿主) + watchdog/tracker 各 0/0。此前监控以 v 系列日志为发现入口，会系统性漏掉 MCP 宿主等非 v 命名进程(典型如 `set_RR11jN_` 服务端任务)；**现改为以 Python 进程为第一视角**，v 日志仅作 scan_script 明细补充。所有进程命令行均无 `RR11jN`(命中 0)，印证其为 WQ BRAIN 服务端句柄而非本机进程名。
...
| v52b_hiring_margin | 在飞扫描 | USA | None | 40 | 16 | 0 | 2.65 | 1.77 | FAIL:M=8.9bp | 优 | 在飞 |

**已完成链合计**：5667 次回测 (各 scan 进程写出的 checkpoint)，26 次 PASS/PASS_CHEAP，1 个 found_alphas。
**在飞合计**：1 个进程，已产出 ≈ 40 条回测结果（实时统计，含 checkpoint 持续写入的任务）。
...
- **本次实证修正**：V46 (已在跑) + V47-V53 (错峰启动于 08:39-08:51) 构成 **8 进程并发、≈5667 次提交、零 429**。说明在"错峰 + 每进程 submit_gate"条件下, 并发进程数可安全 >6; **真正硬约束是瞬时提交浓度 (<=C=7)**, 由每个进程的 gate 共同压住。
...
- **扫描脚本进程 (scan_*.py)**: 2 个 (其中 §1 已发现 1 个有 progress 日志)
...
*报告生成：2026-07-25 21:07:14 GMT+8 · 数据源 results/*_checkpoint.json (进程产物) + *_progress_*.log (运行时核验, 自动发现 V46-V53) + scan_v*.py (源码) + 机器级进程枚举 (Get-CimInstance) · 生成器 gen_report.py 可复跑*