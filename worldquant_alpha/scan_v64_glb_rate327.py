...
LOG_PATH = os.path.join(_HERE, "results", "v64_glb_rate327.log")
...
CKPT = os.path.join(_HERE, "results", "v64_glb_rate327_checkpoint.json")
READY = os.path.join(_HERE, "results", "manual_submit_ready.json")
DIVERSITY = os.path.join(_HERE, "results", "v64_diversity_report.jsonl")
DATASET = "other327"
...
STYLES: Dict[str, List[Tuple[str, str]]] = {
    # ---- v64a: other327 rate-sensitivity (li lv min gan du = yin zi bao lu) ----
    # sirc_lev = dui li lv ping yi (level) min gan; sirc_sl = dui xie lv (slope) min gan
    # fang xiang regime xiang guan -> zheng fu dou tan; VECTOR -> vec_avg ju he
    # A) raw level rank (di TVR tan zhen, liang fang xiang)
    "lvl": [
        ("lvlP_lev", "group_rank(vec_avg(oth327_sirc_lev), market)"),
        ("lvlN_lev", "group_rank(-vec_avg(oth327_sirc_lev), market)"),
        ("lvlP_sl", "group_rank(vec_avg(oth327_sirc_sl), market)"),
        ("lvlN_sl", "group_rank(-vec_avg(oth327_sirc_sl), market)"),
    ],
    # B) beta bian hua/dong liang (ts_delta -> ti sheng TVR fang chu di)
    "dlt": [
        ("dltP_lev22", "group_rank(ts_delta(vec_avg(oth327_sirc_lev), 22), market)"),
        ("dltN_lev22", "group_rank(-ts_delta(vec_avg(oth327_sirc_lev), 22), market)"),
        ("dltP_sl22", "group_rank(ts_delta(vec_avg(oth327_sirc_sl), 22), market)"),
        ("dltP_lev5", "group_rank(ts_delta(vec_avg(oth327_sirc_lev), 5), market)"),
        ("dltN_sl5", "group_rank(-ts_delta(vec_avg(oth327_sirc_sl), 5), market)"),
    ],
    # C) lev vs sl spread (liang zi duan, jian hao zu he)
    "spr": [
        ("sprS_ls", "group_rank(vec_avg(oth327_sirc_lev), market) - group_rank(vec_avg(oth327_sirc_sl), market)"),
        ("sprS_sl", "group_rank(vec_avg(oth327_sirc_sl), market) - group_rank(vec_avg(oth327_sirc_lev), market)"),
    ],
    # D) country gu jia tan zhen
    "cty": [
        ("ctyP_lev", "group_rank(vec_avg(oth327_sirc_lev), country)"),
        ("ctyN_lev", "group_rank(-vec_avg(oth327_sirc_lev), country)"),
        ("ctyP_sl", "group_rank(vec_avg(oth327_sirc_sl), country)"),
    ],
}
    ...
    while True:
        ...
        for item in raw:
            ...
            info = {
                ...
                "tags": ["v64", DATASET, "GLB_D1", "READY_MANUAL", "NO_SUBMIT"],
            }
...
def main():
    ...
    logger.info("V64 GLB other327 rate-sensitivity | %d variants | lanes=%d | %s",
