#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""V56: GLB/D1 未点亮金字塔 analyst15 (Earnings forecasts, pm=1.5) 挖掘.

前序: analyst10 grmfy1 rank 骨架 25 轮(v55c-v55n)证伪 — F/EMEA 结构性反相,
neut 换 INDUSTRY/MARKET 后 AMER 也翻挂(COUNTRY 一直在护 AMER), 全维度无解.
切换 analyst15: 全部为 GICS group/industry/sector 级聚合盈利预测(307 字段,
主力 anl15_gr_12_m_1m_chg a=3280), 行业轮动风格与 analyst10 公司级正交.

结构要点: anl15_gr_* 组内同值 -> INDUSTRY/SUBINDUSTRY neut 会消零信号,
只用 COUNTRY/MARKET; group_rank 无意义, 用全截面 rank.
...
LOG_PATH = os.path.join(_HERE, "results", "v56_glb_analyst15.log")
...
CKPT = os.path.join(_HERE, "results", "v56_glb_analyst15_checkpoint.json")
READY = os.path.join(_HERE, "results", "manual_submit_ready.json")
DIVERSITY = os.path.join(_HERE, "results", "v56_diversity_report.jsonl")
DATASET = "analyst15"
...
# ---------------- 模板族 v56c: 三轮—最强骨架 × universe/neut 全交叉 (最后冲刺) ----------------
# 二轮终局(176变体 found=0): COUN 天花板 S=0.96(b18P_w5) 但 M~6; MARK M 达标但 S~0.75;
#   TOPD 探针只测了 brdU 族: TOPD+MARK 把 brdU_w5 S 0.75->0.89 F 0.56->0.70 M=12.7 —
#   b18P/ibeP 最强骨架从未上过 TOPD/TOP3000, neut 也未试 STATISTICAL.
# 三轮: 4 最强骨架 × {MINVOL1M,TOPDIV3000,TOP3000} × {COUNTRY,MARKET,STATISTICAL},
#   checkpoint label 去重自动跳过已跑组合. 若仍 <1.2 则 analyst15 结案.
STYLES: Dict[str, List[Tuple[str, str]]] = {
    "core4": [
        ("b18P_w5", "rank(ts_mean(ts_backfill(anl15_gr_18_m_ests_up / anl15_gr_18_m_ests, 66), 5))"),
        ("ibeP_w1", "rank(ts_backfill(anl15_ind_12_m_ests_up / anl15_ind_12_m_ests, 66))"),
        ("brdU_w5", "rank(ts_mean(ts_backfill(anl15_gr_12_m_ests_up / anl15_gr_12_m_ests, 66), 5))"),
        ("nbeP_w1", "rank(ts_backfill(anl15_gr_12_m_ests_up - anl15_gr_12_m_ests_dn, 66))"),
    ],
}

UNIVERSES = ["MINVOL1M"]
DECAYS = [0, 3]
# group 级字段组内同值 -> INDUSTRY/SUBI neut 消零; COUNTRY 去国别噪声 / MARKET 保留行业倾斜
NEUTS = ["COUNTRY", "MARKET"]
TRUNCS = [0.02]
MAX_VARIANTS = int(os.environ.get("V53_MAX_VARIANTS", "560"))

# 每风格覆盖 (universe/decay/neut), 未列出的风格沿用全局默认
# core4 全交叉; STATISTICAL 为探针(GLB 若不支持则 error 即证伪, 信息不损失)
STYLE_OVERRIDES: Dict[str, Dict[str, list]] = {
    "core4": {
        "universes": ["MINVOL1M", "TOPDIV3000", "TOP3000"],
        "neuts": ["COUNTRY", "MARKET", "STATISTICAL"],
        "decays": [0],
    },
}
...
def main():
    ...
    logger.info("V56 GLB analyst15 | %d variants | lanes=%d | %s",
