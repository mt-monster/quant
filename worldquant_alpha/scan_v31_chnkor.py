#!/usr/bin/env python3
"""V31: 多区域 PPA 扫描 — ASI/CHN/KOR 未点亮数据集.
...
PROGRESS_LOG_PATH = os.getenv("PROGRESS_LOG_PATH", os.path.join(_HERE, "results", f"v31_chnkor_progress_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"))
...
REGION_CONFIGS = [
    # ASI skipped — ai_news_scores signal too weak (S<0.22)
    ("CHN", "TOP2000U", "continuation_score", [
        ("ascending_channel_similarity_maximum_h40w", "ascending_channel_similarity_middle", "ch_max_vs_mid"),
        ("ascending_channel_similarity_recent", "ascending_channel_similarity_dispersion", "ch_rec_vs_disp"),
        ("ascending_scallop_similarity_average", "ascending_scallop_similarity_minimum", "sc_avg_vs_min"),
        ("ascending_staircase_similarity_maximum", "ascending_staircase_similarity_median_h40w", "st_max_vs_med"),
    ]),
...
logger.info("V31 multi-region scan | %d variants | log: %s", len(VARIANTS), PROGRESS_LOG_PATH)
...
pl = ProgressLogger(total_steps=len(VARIANTS), log_path=PROGRESS_LOG_PATH, task_name="v31_chnkor_region", emit_interval_sec=15.0, max_recent=5)
...
logger.info("V31 complete | %d done, %d valid candidates (NOT submitted)", len(results), len(candidates))
for r in results[:20]:
...
out_path = os.path.join(_HERE, "results", f"scan_v31_chnkor_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
