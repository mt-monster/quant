#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""v56 待跑表达式本地约束校验 (纯本地, 不调 API, 不 import scan 模块避免副作用).

检查项:
  1. 操作符数量 < 6
  2. 禁用算子: trade_when / add() / multiply()
  3. 单表达式数据集字段数 1-2
  4. 字段真实存在于 analyst15 且 coverage > 0.90
...
SCAN = os.path.join(_HERE, "scan_v56_glb_analyst15.py")
FIELDS_JSON = os.path.join(_HERE, "results", "glb_fields_analyst15.json")
...
NEUTS = ["COUNTRY", "MARKET"]; TRUNCS = [0.02]
labels = set()
for style, exprs in styles.items():
    for tag, expr in exprs:
