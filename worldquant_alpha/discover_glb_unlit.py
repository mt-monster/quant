#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""发现 GLB/D1 可用 universe + 未点亮金字塔数据集 (pyramidMultiplier==1.0).

输出: results/glb_unlit_discovery.json
"""
...
logger = logging.getLogger("glb_discover")

from wd_lib_wrapper import WqApiSimple

API = "https://api.worldquantbrain.com"
OUT = os.path.join(_HERE, "results", "glb_unlit_discovery.json")

CANDIDATE_UNIVERSES = ["TOP3000", "MINVOL1M", "ILLIQUID_MINVOL1M", "TOP2000", "TOP1000", "TOP500", "TOPDIV3000"]
...
def main():
    ...
    if not valid_unis:
        logger.error("no valid universe found for GLB D1")
        return

    # 2) 主 universe 拉全部数据集, 记录 pyramidMultiplier
    all_ds = {}
    ...
    for uni, rows in all_ds.items():
        ...
        for x in sorted(unlit, key=lambda v: -(v["fieldCount"] or 0))[:20]:
            logger.info("  UNLIT %s pm=%s fields=%s cov=%s users=%s alphas=%s cat=%s",
                        x["id"], x["pyramidMultiplier"], x["fieldCount"], x["coverage"],
                        x["userCount"], x["alphaCount"], x["category"])
