...
2. **主账号瓶颈 = 信号发现**：7 路 ds 舰队目前 **0 候选**；仅 web_traffic_engage(2.00) 超过 1.58 过闸线，6 路低于线（前三依次 web_traffic_engage 2.00、techindi_model 1.47、equity_kpi_forecast 1.39），整体��号偏弱。
3. **独立账号已有实质候选产出**：594 次仿真全 done，429 条有完整 stats，**146 个通过子宇宙 Sharpe**、18 个被其 ranker 标记可提交——质量显著优于主账号 ds 舰队当前 0 候选，但**仍未走平台提交**。
...
> 7 路在飞舰队首步信号：仅 web_traffic_engage(2.00) 超过 1.58 过闸线但仍卡其他 IS 闸，其余 6 路均低于线——直接印证"信号发现瓶颈"。
...
---
*报告由 build_md_report.py 从真实 checkpoint / progress / tri_track CSV 程序化生成 · 快照 2026-07-25 22:19 · 数字均来自文件实测，未编造。*
