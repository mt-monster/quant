> **数据快照**: 2026-07-25 22:23 GMT+8 ｜ **数据源**: `results/*_checkpoint.json`(权威) + `*_progress_*.log`(实时) + `tri_track_undug_results.csv`

> ⚠️ **提交验证最重要结论**：全部 **35** 个候选 Alpha 仅通过「研究仿真 IS 廉价闸门」，仅 **1** 个跨过生产相关性关（`{found_pid}`，prod_corr={found_pcorr}），**0** 个完成平台真实提交 —— 35 个均不满足 WQ 提交标准，请勿视作可提交 Alpha。
...
| 指标 | 数值 | 说明 |
|---|---|---|
| 累计回测次数 | **6,403** | 全部 32 个 checkpoint 合计 |
...
每一级都是一道硬闸门：**IS 廉价闸门**筛掉 6,367 次（6,367/6403=99.4% 被筛）；**生产相关性关**仅 1 个通过；**平台提交**为 0。36 个候选 ≠ 可提交。
...
| 已提交 alpha | 累计 648 次 (含研究仿真) | 598 alpha 已提交并完成 |
...
|---|---|---|---|---|---|
| equity_kpi_forecast | 96/320 (30.0%) | 🟡 **1.39** | ~84 α/hr | 68 min | 🔴 0 候选 |
| ml_factor_proj | 80/320 (25.0%) | 🔴 **0.36** | ~65 α/hr | 74 min | 🔴 0 候选 |
| order_book_imbalance | 104/320 (32.5%) | 🔴 **0.84** | ~85 α/hr | 74 min | 🔴 0 候选 |
| pv_tech_indicators | 96/320 (30.0%) | 🔴 **0.74** | ~77 α/hr | 75 min | 🔴 0 候选 |
| quant_factor_lib | 88/320 (27.5%) | 🔴 **0.57** | ~73 α/hr | 72 min | 🔴 0 候选 |
| techindi_model | 88/320 (27.5%) | 🟡 **1.47** | ~80 α/hr | 66 min | 🔴 0 候选 |
| web_traffic_engage | 96/320 (30.0%) | 🟢 **2.00** | ~78 α/hr | 74 min | 🔴 0 候选 |
...
| 5 | 🚢 ds_web_traffic_engage_tri_web_traffic_en | 96 | 0 | 🟢 **2.00** | - | gate_S/F/M/Ret |
...
| 9 | 🚢 ds_techindi_model_tri_techindi_model | 88 | 0 | 🟡 **1.47** | - | gate_S/F/M/Ret |
| 10 | 🚢 ds_equity_kpi_forecast_tri_equity_kpi_fo | 96 | 0 | 🟡 **1.39** | - | gate_S/F/M/Ret |
...
| 18 | 🚢 ds_order_book_imbalance_tri_order_book_i | 104 | 0 | 🔴 **0.84** | - | gate_S/F/M/Ret |
...
| 22 | 🚢 ds_pv_tech_indicators_tri_pv_tech_indica | 96 | 0 | 🔴 **0.74** | - | gate_S/F/M/Ret |
...
| 27 | 🚢 ds_quant_factor_lib_tri_quant_factor_lib | 88 | 0 | 🔴 **0.57** | - | gate_S/F/M/Ret |
...
| 已提交 alpha | **598** 个 (全部 status=done) |
| 分轨分布 | explore 345 / improve 149 / misc 104 |
| 时间范围 | 2026-07-25 11:58:04 ~ 2026-07-25 22:22:27 |
| 结果文件 | `tri_track_undug_results.csv` (599 行) |
...
> **PF:子宇宙 Sharpe 是头号平台失败闸门** (298 次)，印证'在子宇宙层面优化中性化/约束'是攻坚方向。IS 指标失败(夏普/拟合/换手/收益)合计 22,556 次，submit_failed 664 次(多为研究仿真本地拒，非平台 429)。
...
1. **候选无一满足提交标准**。真实提交四关：①研究仿真 IS ✅36/36；②生产仿真(OOS) ❌0；③生产相关性 ✅仅 1（YPgAa3WR）；④平台提交 ❌0。PASS_CHEAP ≠ 可提交。
...
5. **提交前必补四关**：生产仿真 → /check(PROD/SELF_CORRELATION) → 平台 submittable → 显式 submit。优先验证 YPgAa3WR。
6. **tri_track 脚本升级**：改输出为 checkpoint 格式(含 Sharpe/Fitness/失败闸门)，纳入统一监控体系。

---
*报告由 `build_md_report.py` 从真实 checkpoint/progress/CSV 文件程序化生成 · 快照 2026-07-25 22:23 GMT+8 · 数字均来自文件实测，未编造。*
