#!/usr/bin/env python3
"""V37: USA D1 other545 (NEMO 2.0, 未点亮 PM=1.5) — 真 multi-sim 8 并发.

接替 V36 stock_cluster_dl (best S~0.57 放弃)。
硬规则同 V36: ops<6, 禁 trade_when/add/multiply/+/*, 不提交。
"""
...
BATCH_SIZE = 8
BATCH_COOLDOWN_SEC = float(os.environ.get("V37_COOLDOWN", "45"))
PROGRESS_LOG_PATH = os.environ.get(
    "PROGRESS_LOG_PATH",
    os.path.join(_HERE, "results", f"v37_other545_progress_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"),
)
CKPT_PATH = os.path.join(_HERE, "results", "v37_other545_checkpoint.json")
FOUND_PATH = os.path.join(_HERE, "results", f"scan_v37_other545_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
DATASET = "other545"

logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
logger = logging.getLogger("v37")
...
# VECTOR signal fields (skip mktcap as primary — size, not alpha)
FIELDS = ["oth545_score", "oth545_mpd", "oth545_pln_omen"]
PAIRS = [
    ("oth545_score", "oth545_mpd", "score_mpd"),
    ("oth545_score", "oth545_pln_omen", "score_omen"),
    ("oth545_mpd", "oth545_pln_omen", "mpd_omen"),
]
...
def build_variants() -> List[Dict[str, Any]]:
    ...
    for f in FIELDS:
        short = f.replace("oth545_", "")[:12]
        # VECTOR → vec_avg → ts_*
        add(f"z_{short}_m22_z66", f"rank(ts_zscore(ts_mean(vec_avg({f}), 22), 66))", "TOP3000", 5, "SUBINDUSTRY", "zscore_mom", f)
        add(f"z_{short}_m66_z252", f"rank(ts_zscore(ts_mean(vec_avg({f}), 66), 252))", "ILLIQUID_MINVOL1M", 5, "SUBINDUSTRY", "zscore_long", f)
        add(f"nz_{short}_m22_z66", f"-rank(ts_zscore(ts_mean(vec_avg({f}), 22), 66))", "TOP3000", 5, "SUBINDUSTRY", "zscore_flip", f)
        add(f"tr_{short}_m22_r126", f"rank(ts_rank(ts_mean(vec_avg({f}), 22), 126))", "TOP3000", 3, "INDUSTRY", "ts_rank", f)
        add(f"d_{short}_m10_d5", f"rank(ts_delta(ts_mean(vec_avg({f}), 10), 5))", "TOP2000", 8, "SUBINDUSTRY", "delta", f)
        add(f"gr_{short}_m22", f"group_rank(ts_mean(vec_avg({f}), 22), industry)", "TOP3000", 5, "SUBINDUSTRY", "group_rank", f)
        add(f"gz_{short}_m22_z66", f"group_zscore(ts_zscore(ts_mean(vec_avg({f}), 22), 66), industry)", "ILLIQUID_MINVOL1M", 3, "INDUSTRY", "group_zscore", f)
        add(f"ir_{short}_m5_66", f"rank(ts_ir(ts_mean(vec_avg({f}), 5), 66))", "TOP3000", 5, "SUBINDUSTRY", "ts_ir", f)
        add(f"sm_{short}_m5", f"rank(ts_mean(vec_avg({f}), 5))", "TOP1000", 5, "SUBINDUSTRY", "short_mean", f)
        add(f"vs_{short}_m22_z66", f"rank(ts_zscore(ts_mean(vec_sum({f}), 22), 66))", "TOP3000", 8, "SUBINDUSTRY", "vec_sum_z", f)
        # raw score without heavy ts (model already scored)
        add(f"raw_{short}_va", f"rank(vec_avg({f}))", "TOP3000", 5, "SUBINDUSTRY", "raw_rank", f)
        add(f"nraw_{short}_va", f"-rank(vec_avg({f}))", "TOP3000", 5, "SUBINDUSTRY", "raw_flip", f)

    for a, b, tag in PAIRS:
        add(f"sp_{tag}_z66", f"rank(ts_zscore(subtract(vec_avg({a}), vec_avg({b})), 66))", "TOP3000", 5, "SUBINDUSTRY", "spread_z", f"{a}|{b}")
        add(f"nsp_{tag}_z66", f"-rank(ts_zscore(subtract(vec_avg({a}), vec_avg({b})), 66))", "TOP3000", 3, "INDUSTRY", "spread_flip", f"{a}|{b}")
        add(f"sp_{tag}_gr", f"group_rank(ts_mean(subtract(vec_avg({a}), vec_avg({b})), 22), industry)", "TOP2000", 8, "SUBINDUSTRY", "spread_group", f"{a}|{b}")

    core = ["oth545_score", "oth545_mpd"]
    for f in core:
        short = f.replace("oth545_", "")
        for uni in UNIVERSES:
            for decay in DECAYS:
                for neut in NEUTS:
                    add(f"grid_z_{short}_{uni}_d{decay}_{neut[:3]}", f"rank(ts_zscore(ts_mean(vec_avg({f}), 22), 126))", uni, decay, neut, "grid_z", f)
                    add(f"grid_nz_{short}_{uni}_d{decay}_{neut[:3]}", f"-rank(ts_zscore(ts_mean(vec_avg({f}), 22), 126))", uni, decay, neut, "grid_nz", f)
                    add(f"grid_raw_{short}_{uni}_d{decay}_{neut[:3]}", f"rank(vec_avg({f}))", uni, decay, neut, "grid_raw", f)
...
def diversity_report(results: List[Dict], variants_meta: Dict[str, Dict], round_n: int):
    ...
    for r in recent:
        ...
        for op in re.findall(r"[a-z_]+\(", expr.lower()):
            ops_used[op[:-1]] += 1
        sk = re.sub(r"oth545_[a-z0-9_]+", "F", expr)
        skeletons[sk[:90]] += 1
    ...
    logger.info(
        "  收益归因: NEMO复合分/动量/NLP网络动量; 预处理=vec_avg+ts平滑;"
        " 失效风险=模型换版/覆盖空洞; 风格=原始分/时序z/价差/行业相对"
    )
    logger.info("=" * 60)
...
def main():
    ...
    logger.info("V37 %s | %d variants | batch=%d | NO SUBMIT", DATASET, len(VARIANTS), BATCH_SIZE)
    ...
    for r in survivors:
        ...
        set_alpha_props(api, pid, f"v37_{DATASET}_{label}", ["v37", DATASET, "USA_D1", "READY_MANUAL", "NO_SUBMIT"])
