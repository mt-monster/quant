#!/usr/bin/env python3
"""V43: USA D1 event_relation (未点亮, cov~0.89) — multi-sim 8.

风格: 文本事件关系强度/类型 (异于 insider/舆情/盈利风险).
...
BATCH_SIZE = 8
BATCH_COOLDOWN_SEC = float(os.environ.get("V43_COOLDOWN", "45"))
PROGRESS_LOG_PATH = os.path.join(_HERE, "results", f"v43_event_rel_progress_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
CKPT_PATH = os.path.join(_HERE, "results", "v43_event_rel_checkpoint.json")
FOUND_PATH = os.path.join(_HERE, "results", f"scan_v43_event_rel_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
READY_PATH = os.path.join(_HERE, "results", "manual_submit_ready.json")
DATASET = "event_relation"
...
FIELDS = [
    "relation_strength_score_general",
    "relation_strength_rank_general",
    "entity_connection_type_general",
    "relation_strength_score_financial",
    "relation_strength_rank_financial",
    "relation_strength_score_partner",
    "relation_strength_rank_partner",
]
PAIRS = [
    ("relation_strength_score_financial", "relation_strength_score_partner", "fin_vs_part"),
    ("relation_strength_score_general", "relation_strength_score_financial", "gen_vs_fin"),
    ("relation_strength_rank_general", "relation_strength_rank_financial", "rgen_vs_rfin"),
]
...
def build_variants():
    ...
    for f in FIELDS:
        short = f.replace("relation_strength_", "rs_").replace("entity_connection_type_", "ect_")[:16]
        for uni in UNIVERSES:
            for decay in DECAYS:
                for neut in NEUTS:
                    add(f"z_{short}_{uni}_d{decay}_{neut[:3]}", f"rank(ts_zscore(ts_mean(vec_avg({f}), 22), 126))", uni, decay, neut, "zscore", f)
                    add(f"nz_{short}_{uni}_d{decay}_{neut[:3]}", f"-rank(ts_zscore(ts_mean(vec_avg({f}), 22), 126))", uni, decay, neut, "zscore_flip", f)
                    add(f"gz_{short}_{uni}_d{decay}_{neut[:3]}", f"rank(group_zscore(ts_zscore(ts_mean(vec_avg({f}), 22), 189), industry))", uni, decay, neut, "group_z", f)
    ...
    for a, b, tag in PAIRS:
        for uni in UNIVERSES:
            for decay in DECAYS:
                for neut in NEUTS:
                    add(f"sp_{tag}_{uni}_d{decay}_{neut[:3]}", f"rank(ts_zscore(subtract(vec_avg({a}), vec_avg({b})), 126))", uni, decay, neut, "spread", f"{a}|{b}")
                    add(f"nsp_{tag}_{uni}_d{decay}_{neut[:3]}", f"-rank(ts_zscore(subtract(vec_avg({a}), vec_avg({b})), 126))", uni, decay, neut, "spread_flip", f"{a}|{b}")
                    add(f"spgz_{tag}_{uni}_d{decay}_{neut[:3]}", f"group_zscore(ts_zscore(subtract(vec_avg({a}), vec_avg({b})), 189), industry)", uni, decay, neut, "spread_gz", f"{a}|{b}")
...
def main():
    ...
    def prio(v):
        s = 0
        if "general" in str(v["field"]): s += 8
        if "score" in str(v["field"]): s += 4
        if v["style"] in ("group_z", "zscore", "spread"): s += 5
        if v["settings"]["universe"] == "TOP3000": s += 3
        return -s
    ...
    logger.info("V43 %s | %d | NO SUBMIT", DATASET, len(VARIANTS))
    ...
    pl = ProgressLogger(total_steps=len(VARIANTS), log_path=PROGRESS_LOG_PATH, task_name="v43_event_rel", emit_interval_sec=15.0, max_recent=8)
    ...
    for r in survivors:
        ...
        if pc_val is None or pc_val >= MAX_PROD_CORR:
            ...
            continue
        info = {"dataset": DATASET, "style": "event_relation_strength", "pid": pid, "label": label, "expr": expr, "sharpe": r["sharpe"], "fitness": r["fitness"], "tvr": r["tvr"], "margin": r["margin"], "prod_corr": pc_val, "risk_neut": rn_stats, "robust": robust, "settings": settings, "submitted": False}
        set_alpha_props(api, pid, f"v43_{label}", ["v43", DATASET, "USA_D1", "READY_MANUAL", "NO_SUBMIT"])
        found_alphas.append(info)
        append_ready({**info, "margin_bp": (r["margin"] or 0) * 10000, "tags": ["v43", DATASET, "USA_D1", "READY_MANUAL"]})
