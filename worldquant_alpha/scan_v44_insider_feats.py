#!/usr/bin/env python3
"""V44: USA D1 insider_feats (未点亮) — multi-sim 8.

风格: 内部人买卖比 (buy/sell ratio), 异于 insider_matrix 的 eur_top_value 动量.
参考 V39 赢家骨架 group_zscore+ts_zscore; ops<6; 禁 add/multiply; 不提交.
"""
...
BATCH_SIZE = 8
BATCH_COOLDOWN_SEC = float(os.environ.get("V44_COOLDOWN", "45"))
PROGRESS_LOG_PATH = os.path.join(_HERE, "results", f"v44_insider_feats_progress_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
CKPT_PATH = os.path.join(_HERE, "results", "v44_insider_feats_checkpoint.json")
FOUND_PATH = os.path.join(_HERE, "results", f"scan_v44_insider_feats_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
READY_PATH = os.path.join(_HERE, "results", "manual_submit_ready.json")
DATASET = "insider_feats"

logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
logger = logging.getLogger("v44")
...
FIELDS = [
    "buy_sell_ratio_all_20d_filled",
    "buy_sell_ratio_all_60d_filled",
    "buy_sell_ratio_all_250d_filled",
    "buy_sell_ratio_top5_60d_filled",
    "buy_sell_ratio_top20_60d_filled",
    "buy_sell_ratio_all_5d_filled",
    "buy_sell_tx_count_ratio_all_60d_filled",
    "buy_sell_tx_count_ratio_all_20d_filled",
]
PAIRS = [
    ("buy_sell_ratio_all_20d_filled", "buy_sell_ratio_all_250d_filled", "bs_20_250"),
    ("buy_sell_ratio_top5_60d_filled", "buy_sell_ratio_all_60d_filled", "top5_vs_all"),
    ("buy_sell_ratio_all_5d_filled", "buy_sell_ratio_all_60d_filled", "bs_5_60"),
]
...
def build_variants():
    ...
    for f in FIELDS:
        short = f.replace("buy_sell_ratio_", "bsr_").replace("buy_sell_tx_count_ratio_", "bscr_").replace("_filled", "")[:18]
        for uni in UNIVERSES:
            for decay in DECAYS:
                for neut in NEUTS:
                    for trunc in TRUNCS:
                        # V39 赢家骨架 (VECTOR 用 vec_avg)
                        add(f"gz_{short}_{uni}_d{decay}_{neut[:3]}_t{int(trunc*100)}",
                            f"rank(group_zscore(ts_zscore(ts_mean(vec_avg({f}), 22), 189), industry))",
                            uni, decay, neut, "group_z", f, trunc)
                        add(f"z_{short}_{uni}_d{decay}_{neut[:3]}_t{int(trunc*100)}",
                            f"rank(ts_zscore(ts_mean(vec_avg({f}), 22), 189))",
                            uni, decay, neut, "zscore", f, trunc)
                    add(f"nz_gz_{short}_{uni}_d{decay}_{neut[:3]}",
                        f"-rank(group_zscore(ts_zscore(ts_mean(vec_avg({f}), 22), 189), industry))",
                        uni, decay, neut, "group_z_flip", f)
                    add(f"raw_{short}_{uni}_d{decay}_{neut[:3]}", f"rank(vec_avg({f}))", uni, decay, neut, "raw", f)
                    add(f"d_{short}_{uni}_d{decay}_{neut[:3]}", f"rank(ts_delta(vec_avg({f}), 10))", uni, decay, neut, "delta", f)
                    add(f"tr_{short}_{uni}_d{decay}_{neut[:3]}", f"rank(ts_rank(vec_avg({f}), 126))", uni, decay, neut, "ts_rank", f)

    for a, b, tag in PAIRS:
        for uni in UNIVERSES:
            for decay in DECAYS:
                for neut in NEUTS:
                    add(f"sp_{tag}_{uni}_d{decay}_{neut[:3]}",
                        f"rank(ts_zscore(subtract(vec_avg({a}), vec_avg({b})), 126))",
                        uni, decay, neut, "spread", f"{a}|{b}")
                    add(f"nsp_{tag}_{uni}_d{decay}_{neut[:3]}",
                        f"-rank(ts_zscore(subtract(vec_avg({a}), vec_avg({b})), 126))",
                        uni, decay, neut, "spread_flip", f"{a}|{b}")
                    add(f"spgz_{tag}_{uni}_d{decay}_{neut[:3]}",
                        f"group_zscore(ts_zscore(subtract(vec_avg({a}), vec_avg({b})), 189), industry)",
...
def evaluate_is(api, label, pid, expr, settings):
    ...
    if s > 1.4 or not fails:
        ...
        if not fails:
            if not ok:
                return {"label": label, "pid": pid, "expr": expr, "settings": settings, "sharpe": s, "fitness": f, "tvr": tvr, "margin": m, "status": "CHECK_PENDING", "fails": ["check_pending"], "sub_univ": sub_v, "sub_limit": sub_lim}
    ...
    return {"label": label, "pid": pid, "expr": expr, "settings": settings, "sharpe": s, "fitness": f, "tvr": tvr, "margin": m, "status": "PASS_CHEAP" if not fails else "FAIL", "fails": fails, "checks": checks, "failed_checks": plat_fails, "sub_univ": sub_v, "sub_limit": sub_lim}
...
def main():
    ...
    def prio(v):
        s = 0
        if "60d" in str(v["field"]) or "20d" in str(v["field"]): s += 8
        if v["style"] in ("group_z", "zscore"): s += 10
        if v["settings"]["universe"] == "TOP3000": s += 4
        if v["settings"]["neutralization"] == "SECTOR": s += 3
        if v["settings"]["decay"] in (2, 3): s += 2
        return -s

    VARIANTS.sort(key=prio)
    if len(VARIANTS) > 200:
        logger.info("Trim %d -> 200", len(VARIANTS))
        VARIANTS = VARIANTS[:200]
    vmeta = {v["label"]: v for v in VARIANTS}
    logger.info("V44 %s | %d | NO SUBMIT", DATASET, len(VARIANTS))
    ...
    pl = ProgressLogger(total_steps=len(VARIANTS), log_path=PROGRESS_LOG_PATH, task_name="v44_insider_feats", emit_interval_sec=15.0, max_recent=8)
    ...
    for bi, batch in enumerate(batches):
        ...
        for r in sorted(results, key=lambda x: _f(x.get("sharpe")) or -999, reverse=True)[:3]:
            if r.get("sharpe") is not None:
                logger.info("  top %s S=%.3f F=%.3f TVR=%.3f sub=%s %s", r.get("label"), r.get("sharpe") or 0, r.get("fitness") or 0, r.get("tvr") or 0, r.get("sub_univ"), r.get("status"))
        for r in results:
            pl.step(extra={"label": r.get("label"), "pid": r.get("pid"), "status": r.get("status"), "sharpe": r.get("sharpe"), "fails": r.get("fails"), "sub_univ": r.get("sub_univ"), "phase": 1, "batch_wall_sec": round(wall, 1)}, force_emit=True)
            ckpt_results.append({"label": r.get("label"), "pid": r.get("pid"), "status": r.get("status"), "sharpe": r.get("sharpe"), "fitness": r.get("fitness"), "tvr": r.get("tvr"), "margin": r.get("margin"), "fails": r.get("fails") or [], "sub_univ": r.get("sub_univ"), "sub_limit": r.get("sub_limit"), "expr": r.get("expr") or (vmeta.get(r.get("label")) or {}).get("expr"), "style": (vmeta.get(r.get("label")) or {}).get("style"), "settings": r.get("settings")})
    ...
    for r in survivors:
        ...
        if pc_val is None or pc_val >= MAX_PROD_CORR:
            ...
            continue
        # 与已有 YPgAa3WR 相关检查留给用户/后续; 此处先收
        info = {"dataset": DATASET, "style": "insider_buy_sell_ratio", "pid": pid, "label": label, "expr": expr, "sharpe": r["sharpe"], "fitness": r["fitness"], "tvr": r["tvr"], "margin": r["margin"], "prod_corr": pc_val, "risk_neut": rn_stats, "robust": robust, "settings": settings, "submitted": False}
        set_alpha_props(api, pid, f"v44_{label}", ["v44", DATASET, "USA_D1", "READY_MANUAL", "NO_SUBMIT"])
        found_alphas.append(info)
        append_ready({**info, "margin_bp": (r["margin"] or 0) * 10000, "tags": ["v44", DATASET, "USA_D1", "READY_MANUAL"]})
