...
LOG_PATH = os.path.join(_HERE, "results", "v63_glb_cnn.log")
...
DIVERSITY = os.path.join(_HERE, "results", "v63_diversity_report.jsonl")
...
STYLES: Dict[str, List[Tuple[str, str]]] = {
    # ---- v63a: chart_cnn_alpha shou lun ----
    # A) q2 duo kong spread (bucket1=up prob, bucket0=down prob)
    "bb": [
        ("bbS_20", "group_rank(ts_mean(img20d_q2_bucket1_prob, 22), market) - group_rank(ts_mean(img20d_q2_bucket0_prob, 22), market)"),
        ("bbS_60", "group_rank(ts_mean(img60d_q2_bucket1_prob, 22), market) - group_rank(ts_mean(img60d_q2_bucket0_prob, 22), market)"),
        ("bbS_80", "group_rank(ts_mean(img80d_q2_bucket1_prob, 22), market) - group_rank(ts_mean(img80d_q2_bucket0_prob, 22), market)"),
        ("bbS_120dn", "group_rank(-ts_mean(img120d_q2_bucket0_prob, 22), market)"),
    ],
    # B) q5 ji duan wei bu spread (bucket4=top, bucket0=bottom)
    "q5t": [
        ("q5tS_20", "group_rank(ts_mean(img20d_q5_bucket4_prob, 22), market) - group_rank(ts_mean(img20d_q5_bucket0_prob, 22), market)"),
        ("q5tS_60", "group_rank(ts_mean(img60d_q5_bucket4_prob, 22), market) - group_rank(ts_mean(img60d_q5_bucket0_prob, 22), market)"),
        ("q5tS_120", "group_rank(ts_mean(img120d_q5_bucket4_prob, 22), market) - group_rank(ts_mean(img120d_q5_bucket0_prob, 22), market)"),
    ],
    # C) dan zi duan tan zhen (fang xiang: up=+ / down=-)
    "sgl": [
        ("sglP_dn60", "group_rank(-ts_mean(img60d_q2_bucket0_prob, 22), market)"),
        ("sglP_up60", "group_rank(ts_mean(img60d_q2_bucket1_prob, 22), market)"),
        ("sglP_t4_60", "group_rank(ts_mean(img60d_q5_bucket4_prob, 22), market)"),
        ("sglP_dn20", "group_rank(-ts_mean(img20d_q2_bucket0_prob, 22), market)"),
    ],
    # D) country gu jia tan zhen
    "cty": [
        ("ctyS_60", "group_rank(ts_mean(img60d_q2_bucket1_prob, 22), country) - group_rank(ts_mean(img60d_q2_bucket0_prob, 22), country)"),
        ("ctyS_20", "group_rank(ts_mean(img20d_q2_bucket1_prob, 22), country) - group_rank(ts_mean(img20d_q2_bucket0_prob, 22), country)"),
    ],
    # ---- v63b: chang zhou qi zu ti su (suo chuang 5/10 + qian decay) ----
    "fw": [
        ("fwS_q120w5", "group_rank(ts_mean(img120d_q5_bucket4_prob, 5), market) - group_rank(ts_mean(img120d_q5_bucket0_prob, 5), market)"),
        ("fwS_q120w10", "group_rank(ts_mean(img120d_q5_bucket4_prob, 10), market) - group_rank(ts_mean(img120d_q5_bucket0_prob, 10), market)"),
        ("fwS_b80w5", "group_rank(ts_mean(img80d_q2_bucket1_prob, 5), market) - group_rank(ts_mean(img80d_q2_bucket0_prob, 5), market)"),
        ("fwS_b80w10", "group_rank(ts_mean(img80d_q2_bucket1_prob, 10), market) - group_rank(ts_mean(img80d_q2_bucket0_prob, 10), market)"),
        ("fwS_d120w5", "group_rank(-ts_mean(img120d_q2_bucket0_prob, 5), market)"),
        ("fwS_d120w10", "group_rank(-ts_mean(img120d_q2_bucket0_prob, 10), market)"),
    ],
    # raw (wu ts_mean) ji su tan zhen
    "fr": [
        ("frS_q120", "group_rank(img120d_q5_bucket4_prob, market) - group_rank(img120d_q5_bucket0_prob, market)"),
        ("frS_b80", "group_rank(img80d_q2_bucket1_prob, market) - group_rank(img80d_q2_bucket0_prob, market)"),
    ],
    # 80d q5 dui zhao (wei shi 80d ji duan tong)
    "f8": [
        ("f8S_q80", "group_rank(ts_mean(img80d_q5_bucket4_prob, 10), market) - group_rank(ts_mean(img80d_q5_bucket1_prob, 10), market)"),
    ],

    # ---- v63c: fei yu ce shu chu yu yi (feature qian ru + confidence) ----
    "ft": [
        ("ftP_f1leap", "group_rank(ts_mean(img_feature1_leapstar1batch1_day1, 10), market)"),
        ("ftP_f2leap", "group_rank(ts_mean(img_feature2_leapstar1_day1, 10), market)"),
        ("ftP_f1malta", "group_rank(ts_mean(img_feat1_malta_star3_d1, 10), market)"),
        ("ftP_f2malta", "group_rank(ts_mean(img_feat2_malta_star2_batch1_d1, 10), market)"),
        ("ftP_f1wolf", "group_rank(ts_mean(img_feature1_wolfegame3_batch2_d1, 10), market)"),
        ("ftN_f1leap", "group_rank(-ts_mean(img_feature1_leapstar1batch1_day1, 10), market)"),
    ],
    "cf": [
        ("cfP_r60q5s4", "group_rank(ts_mean(img_confidence_r60_q5_score4, 10), market)"),
        ("cfP_r60q5s0", "group_rank(-ts_mean(img_confidence_r60_q5_score0, 10), market)"),
        ("cfS_r60q5", "group_rank(ts_mean(img_confidence_r60_q5_score4, 10), market) - group_rank(ts_mean(img_confidence_r60_q5_score0, 10), market)"),
        ("cfP_r30q5s3", "group_rank(ts_mean(img_confidence_r30_q5_score3, 10), market)"),
    ],
...
def main():
    ...
    logger.info("V63 GLB chart_cnn_alpha | %d variants | lanes=%d | %s",
