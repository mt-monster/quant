...
- **数据快照时间**: 2026-07-25 09:24:00 GMT+8
- **覆盖任务**: V33 -> V53 (历史链 V33-V45 + 主账号 trio V46 + 并发批次 V47-V53，共 23 个扫描任务)
- **累计回测次数(已完成链)**: 2843  |  **累计通过候选**: 1  |  **全链路最佳 Sharpe**: 2.58
- **当前在飞 Python 进程**: 8 个 (V46+V47-V53)，已提交在飞回测 ≈ 368 次，全局 429 = 0
...
2. **零 429 实证**：8 进程并发、已提交 ≈368 次回测，全链路 `submit_failed=0 / 429=0 / poll_timeout=0`。并发批次启动时间 **错峰分布在 08:39-08:51 (约12分钟)**，且各进程自带 submit_gate(>=18s/>=45s)，故瞬时提交浓度被压在 C=7 内——**证实"错峰 + 每进程闸门"可安全突破旧保守上限(<=6)**。
...
7. **吞吐量实证 (回测效率)**：8 进程聚合并发吞吐 ≈ **4244 α/hr**，为单进程基线 (V46 ≈ 567 α/hr) 的 **7.5×**；提交方案(multi-sim + gate + 退避)零 429 浪费，**方案层效率 = 优**。当前吞吐由各进程自身 gate 节奏绑定(45–70s/step)，尚未触达平台 compute 饱和；**真正瓶颈是信号发现(Sharpe 低)而非吞吐**——在出候选前加并发只是加速"挖 0 候选"。详见 §6。
...
|---|---|---|---|---|---|---|---|---|
| V46·insider_trx_matrix | 48172 | ALIVE (运行中) | 96/320 | 50.8s/步 | 567 | 0 | 0 | 否 |
| V47·search_interest | 37644 | ALIVE (运行中) | 48/320 | 53.2s/步 | 542 | 0 | 0 | 否 |
| V48·acquisition_model | 35656 | ALIVE (运行中) | 32/320 | 68.5s/步 | 420 | 0 | 0 | 否 |
| V49·forward_beta_risk | 46684 | ALIVE (运行中) | 48/320 | 50.4s/步 | 571 | 0 | 0 | 否 |
| V50·board_network | 52816 | ALIVE (运行中) | 40/320 | 50.0s/步 | 576 | 0 | 0 | 否 |
| V51·behavioral_signals | 46164 | ALIVE (运行中) | 40/320 | 51.2s/步 | 563 | 0 | 0 | 否 |
| V52·hiring_trends | 28176 | ALIVE (运行中) | 40/177 | 52.7s/步 | 547 | 0 | 0 | 否 |
| V53·stock_search_trends | 53492 | ALIVE (运行中) | 24/320 | 63.0s/步 | 458 | 0 | 0 | 否 |
...
| V46·insider_trx_matrix | 主trio | USA | 96 | 96 | 0 | 0 | 0.92 | 0.56 | gate_S/F/M/Ret | 优 | checkpoint |
| V47·search_interest | 并发批 | - | 320 | 48 | 0 | 0 | 1.53 | - | 进行中 | 优 | 在飞 |
| V48·acquisition_model | 并发批 | - | 320 | 32 | 0 | 0 | 0.33 | - | 进行中 | 优 | 在飞 |
| V49·forward_beta_risk | 并发批 | - | 320 | 48 | 0 | 0 | 0.14 | - | 进行中 | 优 | 在飞 |
| V50·board_network | 并发批 | - | 320 | 40 | 0 | 0 | 0.24 | - | 进行中 | 优 | 在飞 |
| V51·behavioral_signals | 并发批 | - | 320 | 40 | 0 | 0 | 1.58 | - | 进行中 | 优 | 在飞 |
| V52·hiring_trends | 并发批 | - | 177 | 40 | 0 | 0 | 2.05 | - | 进行中 | 优 | 在飞 |
| V53·stock_search_trends | 并发批 | - | 320 | 24 | 0 | 0 | 0.92 | - | 进行中 | 优 | 在飞 |

**已完成链合计**：2843 次回测 (各 scan 进程写出的 checkpoint)，10 次 PASS/PASS_CHEAP，1 个 found_alphas。
**在飞合计**：8 个进程，已提交 ≈ 368 次回测 (进度日志实时统计，尚未落 checkpoint)。
...
- 运行时核验：进度日志持续更新 (50.8s/步), 进程存活；96/96 步, 最佳 Sharpe 0.92, 0 个 429。
...
|---|---|---|---|---|---|---|
| V47·search_interest | search_interest | 37644 | 48/320 | 1.53 | 53.2s/步 | 0 |
| V48·acquisition_model | acquisition_model | 35656 | 32/320 | 0.33 | 68.5s/步 | 0 |
| V49·forward_beta_risk | forward_beta_risk | 46684 | 48/320 | 0.14 | 50.4s/步 | 0 |
| V50·board_network | board_network | 52816 | 40/320 | 0.24 | 50.0s/步 | 0 |
| V51·behavioral_signals | behavioral_signals | 46164 | 40/320 | 1.58 | 51.2s/步 | 0 |
| V52·hiring_trends | hiring_trends | 28176 | 40/177 | 2.05 | 52.7s/步 | 0 |
| V53·stock_search_trends | stock_search_trends | 53492 | 24/320 | 0.92 | 63.0s/步 | 0 |
...
| V46·insider_trx_matrix | 运行中 | Y | Y | Y | Y | **优** | 50.8s/步 | ✅运行中·已验证 (PID 48172, 50.8s/步, 429=0) |
| V47·search_interest | 运行中 | Y | Y | Y | Y | **优** (继承V46模板) | 53.2s/步 | ✅运行中·已验证 (PID 37644, 53.2s/步, 429=0) |
| V48·acquisition_model | 运行中 | Y | Y | Y | Y | **优** (继承V46模板) | 68.5s/步 | ✅运行中·已验证 (PID 35656, 68.5s/步, 429=0) |
| V49·forward_beta_risk | 运行中 | Y | Y | Y | Y | **优** (继承V46模板) | 50.4s/步 | ✅运行中·已验证 (PID 46684, 50.4s/步, 429=0) |
| V50·board_network | 运行中 | Y | Y | Y | Y | **优** (继承V46模板) | 50.0s/步 | ✅运行中·已验证 (PID 52816, 50.0s/步, 429=0) |
| V51·behavioral_signals | 运行中 | Y | Y | Y | Y | **优** (继承V46模板) | 51.2s/步 | ✅运行中·已验证 (PID 46164, 51.2s/步, 429=0) |
| V52·hiring_trends | 运行中 | Y | Y | Y | Y | **优** (继承V46模板) | 52.7s/步 | ✅运行中·已验证 (PID 28176, 52.7s/步, 429=0) |
| V53·stock_search_trends | 运行中 | Y | Y | Y | Y | **优** (继承V46模板) | 63.0s/步 | ✅运行中·已验证 (PID 53492, 63.0s/步, 429=0) |
...
- **本次实证修正**：V46 (已在跑) + V47-V53 (错峰启动于 08:39-08:51) 构成 **8 进程并发、≈368 次提交、零 429**。说明在"错峰 + 每进程 submit_gate"条件下, 并发进程数可安全 >6; **真正硬约束是瞬时提交浓度 (<=C=7)**, 由每个进程的 gate 共同压住。
...
|---|---:|---:|---:|
| V46·insider_trx_matrix | 8 | 50.8 | 567 |
| V47·search_interest | 8 | 53.2 | 542 |
| V48·acquisition_model | 8 | 68.5 | 420 |
| V49·forward_beta_risk | 8 | 50.4 | 571 |
| V50·board_network | 8 | 50.0 | 576 |
| V51·behavioral_signals | 8 | 51.2 | 563 |
| V52·hiring_trends | 8 | 52.7 | 547 |
| V53·stock_search_trends | 8 | 63.0 | 458 |
| **聚合 (8 进程并发)** | - | - | **4244** |

- **聚合并发吞吐**：8 进程合计 ≈ **4244 α/hr**（≈ 70.7 α/min）。
- **单进程基线**：V46 单独 ≈ 567 α/hr；8 进程并发 = **7.5× 单进程吞吐**，接近线性加速（8 进程→7.5×，因各进程 gate 节奏 45–70s 略有差异，未完全同步）。
...
- **核心瓶颈 = 信号发现，不是吞吐**：并发批次首步 Sharpe 仅 -0.14~1.94，远低于 1.25 闸门。在出候选前，4244 α/hr 的算力只是加速「挖出 0 候选」。若把同等算力转向已验证的 V39b 风格扩展 (低自相关 + 行业中性 + W189/d3/SECTOR/t1)，出候选概率更高——**吞吐已不是限制因素，范式转向才是**。<br>*附：历史链 V33-V45 完成期吞吐约 40–80 α/hr/进程 (见 §4.2 节奏列)，与当前并发批次同量级，说明单进程效率长期稳定，提升来自并发叠加而非单进程优化。*
...
- **V46 ETA**：2026-07-25T12:33:09 (受 submit_gate 限速)。
...
---
*报告生成：2026-07-25 09:24:00 GMT+8 · 数据源 results/*_checkpoint.json (进程产物) + *_progress_*.log (运行时核验, 自动发现 V46-V53) + scan_v*.py (源码) · 生成器 gen_report.py 可复跑*