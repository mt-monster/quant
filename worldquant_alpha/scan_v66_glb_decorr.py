...
LOG_PATH = os.path.join(_HERE, "results", "v66_glb_decorr.log")
...
BATCH_SIZE = 8
N_LANES = int(os.environ.get("V53_LANES", "2"))  # 用户指示: 8 个 lane 先只开 7 个
COOLDOWN = float(os.environ.get("V53_COOLDOWN", str(DEFAULT_COOLDOWN_SEC)))
CKPT = os.path.join(_HERE, "results", "v66_glb_decorr_checkpoint.json")
READY = os.path.join(_HERE, "results", "manual_submit_ready.json")
DIVERSITY = os.path.join(_HERE, "results", "v66_diversity_report.jsonl")
...
STYLES: Dict[str, List[Tuple[str, str]]] = {
    # ---- v66: starmine egrL qu xiang guan bian huan (ji zuo S=1.76, PC=0.81 -> mu biao <0.7) ----
    # A) huan zhong xing hua (biao da shi = ji zuo, neuts zai OVERRIDES)
    "nt": [
        ("ntP_q60L", "group_rank(ts_decay_linear(ts_backfill(mean_estimate_change_pct_fq1_earnings_60d_4, 22), 10), market)"),
        ("ntP_y60L", "group_rank(ts_decay_linear(ts_backfill(mean_estimate_change_pct_fy1_earnings_60d_4, 22), 10), market)"),
    ],
    # B) huan fen zu: sector/subindustry zu nei pai ming
    "grp": [
        ("grpP_q60sec", "group_rank(ts_decay_linear(ts_backfill(mean_estimate_change_pct_fq1_earnings_60d_4, 22), 10), sector)"),
        ("grpP_q60sub", "group_rank(ts_decay_linear(ts_backfill(mean_estimate_change_pct_fq1_earnings_60d_4, 22), 10), subindustry)"),
        ("grpP_y60sec", "group_rank(ts_decay_linear(ts_backfill(mean_estimate_change_pct_fy1_earnings_60d_4, 22), 10), sector)"),
    ],
    # C) ts_rank shi xu pai ming (xin xi zhou xuan zhuan, yu jie mian rank di xiang guan)
    "tsr": [
        ("tsrP_q60_66", "group_rank(ts_rank(ts_backfill(mean_estimate_change_pct_fq1_earnings_60d_4, 22), 66), market)"),
        ("tsrP_q60_126", "group_rank(ts_rank(ts_backfill(mean_estimate_change_pct_fq1_earnings_60d_4, 22), 126), market)"),
        ("tsrP_y60_66", "group_rank(ts_rank(ts_backfill(mean_estimate_change_pct_fy1_earnings_60d_4, 22), 66), market)"),
    ],
    # D) ts_zscore shi xu biao zhun hua
    "tsz": [
        ("tszP_q60_66", "group_rank(ts_zscore(ts_backfill(mean_estimate_change_pct_fq1_earnings_60d_4, 22), 66), market)"),
        ("tszP_y60_66", "group_rank(ts_zscore(ts_backfill(mean_estimate_change_pct_fy1_earnings_60d_4, 22), 66), market)"),
    ],
    # E) group_neutralize xian shi can cha hua (kou chu sector/industry qing xie)
    "gn": [
        ("gnP_q60sec", "group_neutralize(group_rank(ts_decay_linear(ts_backfill(mean_estimate_change_pct_fq1_earnings_60d_4, 22), 10), market), sector)"),
        ("gnP_q60ind", "group_neutralize(group_rank(ts_decay_linear(ts_backfill(mean_estimate_change_pct_fq1_earnings_60d_4, 22), 10), market), industry)"),
        ("gnP_y60sec", "group_neutralize(group_rank(ts_decay_linear(ts_backfill(mean_estimate_change_pct_fy1_earnings_60d_4, 22), 10), market), sector)"),
    ],
    # F) xiu zheng jia su du (ts_delta, xin hao shi dian cuo kai)
    "acc": [
        ("accP_q60d22", "group_rank(ts_delta(ts_backfill(mean_estimate_change_pct_fq1_earnings_60d_4, 22), 22), market)"),
        ("accP_y60d22", "group_rank(ts_delta(ts_backfill(mean_estimate_change_pct_fy1_earnings_60d_4, 22), 22), market)"),
    ],
    # ---- v66b: grpP_q60sub d3 PC=0.667<0.7 且 S=1.78 突破! 缺口仅 F=0.86.
    #      signed_power 尾部集中 = v59g 验证过 F 0.93->1.00 的杠杆, 叠加细分组 ----
    "pw": [
        ("pwP_q60sub", "signed_power(group_rank(ts_decay_linear(ts_backfill(mean_estimate_change_pct_fq1_earnings_60d_4, 22), 10), subindustry) - 0.5, 2)"),
        ("pwP_q60sec", "signed_power(group_rank(ts_decay_linear(ts_backfill(mean_estimate_change_pct_fq1_earnings_60d_4, 22), 10), sector) - 0.5, 2)"),
        ("pwP_q60ind", "signed_power(group_rank(ts_decay_linear(ts_backfill(mean_estimate_change_pct_fq1_earnings_60d_4, 22), 10), industry) - 0.5, 2)"),
        ("pwP_y60sub", "signed_power(group_rank(ts_decay_linear(ts_backfill(mean_estimate_change_pct_fy1_earnings_60d_4, 22), 10), subindustry) - 0.5, 2)"),
        ("pwP_q60sub15", "signed_power(group_rank(ts_decay_linear(ts_backfill(mean_estimate_change_pct_fq1_earnings_60d_4, 22), 15), subindustry) - 0.5, 2)"),
        ("grpP_q60ind", "group_rank(ts_decay_linear(ts_backfill(mean_estimate_change_pct_fq1_earnings_60d_4, 22), 10), industry)"),
    ],
}
...
STYLE_OVERRIDES: Dict[str, Dict[str, list]] = {
    # nt: 换中性化是本族唯一变量 (COUNTRY 基线已知 PC=0.81, 不重跑)
    "nt": {"neuts": ["SUBINDUSTRY", "STATISTICAL"], "decays": [3, 6]},
    "grp": {"decays": [3, 6]},
    "tsr": {"decays": [3, 6]},
    "tsz": {"decays": [3, 6]},
    "gn": {"decays": [3, 6]},
    ...
    while True:
        ...
        for item in raw:
            ...
            info = {
                ...
                "tags": ["v66", DATASET, "GLB_D1", "READY_MANUAL", "NO_SUBMIT"],
            }
...
def main():
    ...
    logger.info("V66 GLB starmine egrL de-correlation | %d variants | lanes=%d | %s",
